...
Cypress.Commands.add("login", (email, password) => {
    cy.visit('https://www.hesk.com/demo/admin/')  // Launch the application
    cy.title().should('eq', 'Help Desk Demo - Welcome! Please log in.')
    cy.location('protocol').should('eq', 'https:')
    cy.get('input#regInputUsername').clear().type(email)    // chain of commands
    cy.get('input#regInputPassword').clear().type(password)
